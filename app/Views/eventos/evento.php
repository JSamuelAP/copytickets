...
<section class="mt-3 mb-4">
    <img src="<?= base_url($cartelera['imagen']) ?>" alt="Banner de pxndx" class="img-fluid rounded-4 w-100 object-fit-cover" style="max-height: 400px;">
</section>
    ...
    <section class="col">
        <div class="d-flex justify-content-between align-items-center">
            ...
            <?php if (isset($_SESSION['datos']['rol']) && $_SESSION['datos']['rol'] == 2 && $_SESSION['datos']['id'] == $cartelera['organizador_id']) : ?>
                <a href="<?= $cartelera['id'] ?>/editar" class="btn btn-primary btn-small">
                    <i class="bi bi-pen-fill"></i>
                    Editar
                </a>
            <?php endif; ?>
        </div>
        <a href="<?= base_url('public/organizador/perfil/' . $cartelera['organizador_id']) ?>" class="fs-5"><?= $cartelera['nombre'] ?></a>
        ...
        <?php if ($cartelera['fecha'] > date('Y-m-d')) : ?>
            <div class="d-grid">
                <?php if (isset($_SESSION['datos']['rol']) && $_SESSION['datos']['rol'] <= 1) : ?>
                    <button type="button" data-bs-toggle="modal" data-bs-target="#exampleModal" class="btn btn-lg btn-primary">
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form id="pagarEvento" action="<?= base_url('public/pagar/boleto') ?>" method="post" class="mb-5">
                                        <div class="form-group">
                                            <label for="nombreEvento">Nombre del evento:</label>
                                            <input type="text" class="form-control" id="nombreEvento" value="<?= $cartelera['nombre'] ?>" disabled>
                                        </div>
                                        <div class="form-group">
                                            <label for="ubicacion">Ubicación:</label>
                                            <input type="text" class="form-control" id="ubicacion" value="<?= $cartelera['ubicacion'] ?>" disabled>
                                        </div>
                                        <div class="form-group">
                                            <label for="fecha">Fecha:</label>
                                            <input type="date" class="form-control" id="fecha" name="fecha" value="<?= $cartelera['fecha'] ?>" disabled>
                                        </div>
                                        <div class="form-group">
                                            <label for="hora">Hora:</label>
                                            <input type="time" class="form-control" id="hora" name="hora" value="<?= $cartelera['hora'] ?>" disabled>
                                        </div>
                                        <div class="form-group">
                                            <label for="nombreComprador">Nombre del Comprador:</label>
                                            <input type="text" class="form-control" id="nombreComprador" value="<?= $_SESSION['datos']['nombre'] ?>" disabled>
                                        </div>
                                        <div class="form-group">
                                            <label for="numBoletos">Cantidad de boletos:</label>
                                            <input type="number" class="form-control" id="numBoletos" name="numBoletos" value="1" min="1">
                                        </div>
                                        <div class="form-group">
                                            <label for="total">Total:</label>
                                            <input type="text" class="form-control" id="total" value="<?= $cartelera['precio'] ?>" disabled>
                                        </div>
                                        <input type="hidden" id="totalHidden" name="total" value="<?= $cartelera['precio'] ?>">
                                        <input type="hidden" name="precio" value="<?= $cartelera['precio'] ?>">
                                        <input type="hidden" name="usuario_id" value="<?= $_SESSION['datos']['id'] ?>">
                                        <input type="hidden" name="evento_id" value="<?= $cartelera['id'] ?>">
                                        <input type="hidden" name="fecha" value="<?= $cartelera['fecha'] ?>">
                                        <input type="hidden" name="hora" value="<?= $cartelera['hora'] ?>">

                                    </form>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button>
                                    <button type="submit" form="pagarEvento" class="btn btn-primary">Confirmar Compra</button>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <section>
        <?php if (isset($_SESSION['datos']['rol']) && $_SESSION['datos']['rol'] == 2 && $_SESSION['datos']['id'] == $cartelera['organizador_id']) : ?>
            <p class="text-center mb-4">
                <a href="<?= $_SESSION['datos']['id'] ?>/estadisticas" class="link-primary">Ver estadísticas</a>
            </p>
            <p class="fs-5 mb-0">Usuario escáner: </p>
            <div class="mb-2">
                <label class="form-label mb-0 fw-light">Nombre de usuario</label>
                <input type="text" value="<?= $escaner_usuario['usuario'] ?>" class="form-control form-control-sm" disabled readonly>
            </div>
            <div>
                <label class="form-label mb-0 fw-light">Contraseña</label>
                <input type="text" value="<?= $escaner_usuario['password'] ?>" class="form-control form-control-sm" disabled readonly>
            </div>
        <?php endif; ?>
    </section>
    </aside>
